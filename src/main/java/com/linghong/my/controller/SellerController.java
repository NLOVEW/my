...
public class SellerController {
    ...
    public Response updateBusinessMessage(@RequestParam(required = false) String businessImage,
                                          ...
                                          @RequestParam(required = false) String foodLicense,
                                          HttpServletRequest request) {
        Seller seller = sellerService.updateBusinessMessage(businessImage, businessLicense, base64IdCard, foodLicense, request);
        return new Response(true, 200, seller, "完善成功");
    }
     ...
     * @return
     */
    @PostMapping("/seller/uploadLogo")
    public Response uploadLogo(String logoBase64,HttpServletRequest request){
        Long sellerId = JwtUtil.getSellerId(request);
        boolean flag = sellerService.uploadLogo(logoBase64,sellerId);
        if(flag){
